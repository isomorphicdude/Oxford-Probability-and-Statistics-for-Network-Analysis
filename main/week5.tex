...
\begin{definition}
    A \textbf{pure growth (Yule) process} $S(t)$ is a continuous-time Markov chain with state space $\mathbb{N}$ with rate $\rho$ if it satisfies $S(0) = k \geq 1$ and
    \begin{equation*}        
        S(t) = \sum_{i=1}^k N_t^{(i)}
    \end{equation*}
...
In the current context, the pure growth process $S(t)$ should have a similar behaviour to the liquid (we only consider starting from $P$ and ignore $Q$ for now):
