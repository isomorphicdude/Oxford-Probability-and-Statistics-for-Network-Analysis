...
\end{equation*}
where $\pi_i$ is the probability of selecting unit $i$ into the sample. Note this has an alternative for of $\sum_{i\in U} Z_i\frac{y_i}{\pi_i}$, where $Z_i=\mathbf{1}(i\in S)$ is the indicator function, taking the expectation shows that this is an unbiased estimator of $\mu$.  The variance of the estimator is:
\begin{equation*}
...
We need to define the population to be the quantity of interest. Here for network ${\mathcal{G}}=(\mathcal{V},{\mathcal{E}})$, we let the population be all vertices and verterx pairs:
\begin{equation*}
...
Note those probabilities assume the size of population is known.  
...
We also note that other quantities of interest can be estimated in a similar way \begin{example}[Triangles]
...
\begin{enumerate}
    ...
    \item Simulate random sample $T_1, \ldots, T_n$ from the null distribution.
...
Recall a network is scale-free if the degree distribution follows a power law:
\begin{equation*}
...
\paragraph{Girvan-Newman algorithm} The algorithm is based on the idea that edges with high betweenness are likely to be between communities (recall betweenness is the number of shortest paths passing through an edge).
